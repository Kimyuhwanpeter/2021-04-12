...
def tr_func(img_list, lab_list):
    ...
    gaussian_label = tf.one_hot(lab_list, 90)   # [0,0,1,0,0,....]
    
    #gaussian_label = tf.exp(-(gaussian_label - float(lab_list))**2 / 2) * (1 / tf.sqrt(2*3.14))
    gaussian_buf = []
    for i in range(90):
        gaussian_buf.append(tf.exp(-(float(i) - float(lab_list))**2 / 2) * (1 / tf.sqrt(2*3.14)))
    gaussian_buf = tf.convert_to_tensor(gaussian_buf)

    return img, lab, generation, lab_list - 2, gaussian_buf
...
def cal_loss(model, images, age_labels, gener_labels, all_age_label, all_age_onehot, gaussian_label):
    total_loss = 0.
    with tf.GradientTape() as tape:
        ...
        logits2 = tf.nn.softmax(logits2, 1)
        distribution_loss = tf.keras.losses.KLDivergence()(gaussian_label, logits2)
        
        total_loss2 = (distribution_loss*10) + total_loss
...
def cal_mae(model, images, labels):
    ...
    for i in range(137):
        ...
        age_predict2 = tf.nn.softmax(logits2[i])    # 이 부분을 바꿔줘야 함
        age_predict2 = tf.argmax(age_predict2)
        age_predict2 = tf.cast(age_predict2, tf.int32)
    ...
    if FLAGS.train:
        ...
        for epoch in range(FLAGS.epochs):
            ...
            for step in range(tr_idx):
                batch_images, batch_labels, batch_age_gener, real_age_label, gaussian_label = next(tr_iter)
                batch_labels = make_label_V2(batch_labels)
                all_age_onehot = make_label(real_age_label)

                loss = cal_loss(model, batch_images, batch_labels, batch_age_gener, real_age_label, all_age_onehot, gaussian_label)
